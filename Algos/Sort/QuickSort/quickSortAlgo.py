...
ls[0]...ls[i] | pivot | ls[i+1]...ls[j] | ls[j+1]...ls[n-1]
<------------- processed --------------> <-- unprocessed -->

When all of ls is processed, place pivot in correct position:
swap(ls[i+1], ls[j+1])
"""
...
def partition(ls, lo, hi):
    ...
    for j in range(lo, hi):
        if ls[j] <= pivot: # found smaller val: expand small region and swap(small region tail + 1, curr)
            i += 1
            ls[j], ls[i] = ls[i], ls[j]
    ls[i+1], ls[j+1] = ls[j+1], ls[i+1] # swap in pivot with head of bigger region, bc pivot resided in it's tail
    return i+1
